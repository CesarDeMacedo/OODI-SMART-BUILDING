...
### Portfolio Authorship — Technology Stack Column (pre-Stage 10 refinement)

**Status:** Complete  
**Closed:** 2026-06-25  
**Commit:** `a1f674a`

Added a two-column layout to the "Project authorship" card on the Data Transparency page:

* left column: existing authorship text, role summary, disclaimer and CTAs — unchanged;
* right column: "Technology stack" heading, introductory sentence, and a categorised `dl` covering Frontend, Styling & UI, Public data, Visualization, Testing & QA, Deployment and AI workflow;
* responsive: side by side on wider viewports via CSS container query; stacked on narrow viewports;
* AI tools (ChatGPT, Claude Code, Codex) are explicitly labelled as development-assistance tools only — a stack note confirms no generative AI runs inside the deployed application and that insights are generated by deterministic rules;
